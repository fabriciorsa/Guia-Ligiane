...
const Navigation = ({ scrollY }: NavigationProps) => {
    ...
    return (
        ...
        >
            ...
            {/* Mobile Menu Modal */}
            {/* Mobile Menu - Full Screen White Overlay (Maximum Visibility) */}
            {isMobileMenuOpen && (
                <div className="md:hidden fixed inset-0 z-[9999] bg-white animate-fade-in flex flex-col">
                        ...
                        {navLinks.map((link) => {
                            ...
                            return (
                                <button
                                    key={link.name}
                                    onClick={() => handleNavClick(link.href)}
                                    className="flex items-center gap-6 px-4 py-4 rounded-2xl hover:bg-[#F5F0E8] group transition-all duration-200"
                                >
